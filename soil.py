...
class Plant(pygame.sprite.Sprite):
	def __init__(self, plant_type, groups, soil, check_watered):
		...
		self.frames = import_folder(f'../pygame_test/graphics/fruit/{plant_type}')
		self.soil = soil
...
class SoilLayer:
	def __init__(self, all_sprites, collision_sprites):
		...
		# graphics
		self.soil_surfs = import_folder_dict('../pygame_test/graphics/soil/')
		self.water_surfs = import_folder('../pygame_test/graphics/soil_water')

		self.create_soil_grid()
		self.create_hit_rects()

		# sounds
		self.hoe_sound = pygame.mixer.Sound('../pygame_test/audio/hoe.wav')
		self.hoe_sound.set_volume(0.1)

		self.plant_sound = pygame.mixer.Sound('../pygame_test/audio/plant.wav')
		self.plant_sound.set_volume(0.1)

	def create_soil_grid(self):
		ground = pygame.image.load('../pygame_test/graphics/world/ground.png')
		h_tiles, v_tiles = ground.get_width() // TILE_SIZE, ground.get_height() // TILE_SIZE
		
		self.grid = [[[] for col in range(h_tiles)] for row in range(v_tiles)]
		for x, y, _ in load_pygame('../pygame_test/data/map.tmx').get_layer_by_name('Farmable').tiles():
